...
class Tweet(snscrape.base.Item):
	...
	def insert_tweet(self,db,
		...
		try:
			...
			new_media = []
			for i in media:
				if i.__class__  == Photo:
					new_media.append({
						"url" : i.fullUrl,
						"type" : "photo"
					})
				elif i.__class__ == Video:
					new_media.append({
						"type" : "video",
						"duration_ms": i.duration,
						"variants":[{"url": k.url, "bitrate":k.bitrate,"content_type":k.contentType} for k in i.variants],
						"public_metrics" : {
							"view_count" : i.views
						},
			...
			if self.place:
				place = {
					...
					"id" : self.place.id,
					"country_code" : self.place.countryCode,
					"country" : self.place.country,
					"full_name" : self.place.fullName,
			...
			tweet = {
				...
				"tweet_url":self.url,
				"mentionedUsers":[{"userId":str(x.id),"userName":x.username} for x in self.mentionedUsers] if self.mentionedUsers else []
			}

			old_tweet = db.tweets.find_one_and_update({"_id":tweet_id},{"$set":tweet,"$addToSet":{"campaigns":campaigns,"trends":trend,"trend_ids":trend_id}},upsert=True)
			print("tweet added!!!")
			if old_tweet:
				like_count = tweet['like_count'] - old_tweet['like_count']
				retweet_count = tweet['retweet_count']- old_tweet['retweet_count']
				impression_count = tweet['impression_count']- old_tweet['impression_count']
			else:
